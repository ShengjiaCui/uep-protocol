...
| **A2 纵深回填** | **28/106 已实测全适配**（下表 ✓ 标注；另有 BeaverTails/中文字谜等适配集不在本普查名单内）——从桌面普查到真实切片跑通的闭环 |
...
| 44 | IFEval | ✓ | qa | partial | apache-2.0 | en | programmatic (strict/loose acc) | lmeval-cfg | high | 可验证指令约束，Python 程序校验非 LLM 评分；verifiable-reward 缺口；A2 实测适配精化为 custom+§8 instruction_following（桌面 qa/partial 偏乐观，真适配揭示需新 verifier） |
