...
- **Q6（本卡附加裁决）** 中文真实 choices 切片：接入 CC-BY-NC 数据集（CMMLU/C-Eval，注明非商业限制）／ 另寻宽松许可中文数据集 ／ 暂以合成中文夹具过渡？
  答：**已裁决并落地**——C-Eval 已接入（NC 从严：断言集本地全量运行、无黄金入库、锁如实记录），与合成中文夹具并存；宽松许可中文真实数据持续物色（检索原型侧已落 T2Ranking，Apache-2.0）。

> **第一轮评审性质注记**：Q1–Q4 未逐项作答。终签依据 = 6 个 choices 真实数据集
> （含中文 C-Eval）604 条实战验证 + 试金石断言集与黄金文件全绿 + 8 集实跑分数轮廓
> 吻合（docs/validation-report-2026-07.md）。外部领域评审人第二轮随同行征询
> （外部接触点②）进行，届时按模板另行作答 Q1–Q4。

## 4. 评审人：jason（用户本人；领域背景：AI 从业者 / 项目负责人）　日期：2026-07-06

## 5. 用户终签：**jason，2026-07-06**（会话指令原文："签字 Choices 评审卡"）
